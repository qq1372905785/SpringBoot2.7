...
//    @Bean
    ...
    /**
     *ConditionOnBean(name = "BeanName")当容器中存在指定的Bean，才运行下面的@Bean
     *也就是当容器中存在user01这个bean的时候，才会通过@Bean注解把dog01()的bean注册到容器中
     * **/
    @ConditionalOnBean(name = "user01") //条件装配
    @Bean
    public Dog dog01() {
        return new Dog("xiaomi", 7);
    }
    /**
     *ConditionalOnMissingBean(name = "BeanName")当容器中不存在指定的Bean，才运行下面的@Bean
     *也就是当容器中不存在user01这个bean的时候，才会通过@Bean注解把dog02()的bean注册到容器中
     * **/
    @ConditionalOnMissingBean(name = "user01") //条件装配
    @Bean
    public Dog dog02() {
        return new Dog("xiaomi", 7);
    }
